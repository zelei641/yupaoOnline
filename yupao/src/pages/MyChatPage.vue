...
let wsUrl = "ws://localhost:8081/api/chat";
 ...
 */
onMounted( () => {
  let {id, username, avatarUrl} = route.query
  stats.value.id = Number.parseInt(id)
...
const getCurrentPageUser = async () => {
  ...
  beforeMessages.value = await MyAxios.get('/chat/private/get',{
    params: {
      fromId: stats.value.id,
      toId: user.value.id,
    }
  });

  if (beforeMessages.value?.code == 0)
  {
    beforeMessages.value.data.forEach(data => {

      if (data.from_id === stats.value.id)
      {
        let html = `
     <div class="message other">
        <img :alt=${stats.value.username} class="avatar"  src=${stats.value.avatarUrl}>
      <div class="info">
...
function initEventHandle() {
  ...
  ws.onmessage = function (event) {    //如果获取到消息，心跳检测重置
    ...
    if (data.value.code === 0 && data.value.fromUserId === stats.value.id)
    {
      let html = `
     <div class="message other">
        <img :alt=${stats.value.username} class="avatar"  src=${stats.value.avatarUrl}>
